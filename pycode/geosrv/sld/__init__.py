...
def write_sld(attr_name, attr_colors, mapAttrKeys, sld_path,
    ...
    * category -> attr_colors key/column with category value
    ...
    from pycode.geosrv.sld.rules import get_categorical_rules
    from pycode.geosrv.sld.rules import get_quantitative_rules
    ...
    if type(attr_colors) != list:
        if os.path.exists(attr_colors):
            ff = os.path.splitext(attr_colors)[1]
            ...
            if ff == '.json':
                ...
                from pycode.rd import tbl_to_obj
...
def write_raster_sld(attrProp, outSld, dataType="CATEGORICAL"):
    ...
    * FLOATING;
    ...
    from pycode.clr import rgb_to_hex
    from pycode.Xml import write_xml_tree
